...
data{
  ...
  matrix[S,5] X; //(1,lat,log,elev,coast)

  // hyperparameters
  real<lower = 0> sg0prior[2]; // a, b of inverse gamma for gamma
  real<lower = 0> sd0prior[2]; // a, b of inverse gamma for delta
  vector[5] ma0prior; // mean of the normal for gamma
  vector<lower = 0>[5] sa0prior; // sd of the normal for gamma
  vector[5] mn0prior; // mean of the normal for delta
  vector<lower = 0>[5] sn0prior; // sd of the normal for delta
  vector[5] mb0prior; // mean of the normal for lambda
  vector<lower = 0>[5] sb0prior; // sd of the normal for lambda
}
...
parameters{
  ...
  vector[5] ag; // regression parameters for gamma
  vector[5] nd; // regression parameters for delta
  vector[5] bl; // regression parameters for lambda
}
...
model{
  ...
  bl[4] ~ normal(mb0prior[4], sb0prior[4]);
  bl[5] ~ normal(mb0prior[5], sb0prior[5]); 
  ag[1] ~ normal(ma0prior[1], sa0prior[1]);
  ag[2] ~ normal(ma0prior[2], sa0prior[2]);
  ag[3] ~ normal(ma0prior[3], sa0prior[3]);
  ag[4] ~ normal(ma0prior[4], sa0prior[4]);
  ag[5] ~ normal(ma0prior[5], sa0prior[5]);
  nd[1] ~ normal(mn0prior[1], sn0prior[1]);
  nd[2] ~ normal(mn0prior[2], sn0prior[2]);
  nd[3] ~ normal(mn0prior[3], sn0prior[3]);
  nd[4] ~ normal(mn0prior[4], sn0prior[4]);
  nd[5] ~ normal(mn0prior[5], sn0prior[5]);
